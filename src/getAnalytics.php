<?php
	...
	$sql3 = "select name as product_name, url as product_image, avg(rating_1) as rating1 from review r join product p on r.product_id = p.id group by product_id order by rating1 desc limit 5";
	$sql4 = "select name as product_name, url as product_image, avg(rating_2) as rating2 from review r join product p on r.product_id = p.id group by product_id order by rating2 desc limit 5";
	$sql5 = "select name as product_name, url as product_image, avg(rating_3) as rating3 from review r join product p on r.product_id = p.id group by product_id order by rating3 desc limit 5";
